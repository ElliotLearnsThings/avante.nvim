...
- **Turn handling.** `stopReason` values other than `end_turn` are surfaced in the chat; permission requests from parallel tool calls are queued and shown one at a time under the tool call they belong to; `:AvanteStop` answers every pending request with "cancelled"; tool calls that never completed are closed when the turn ends; messages typed while the agent works are queued and sent one per turn; only the new message is sent each turn instead of replaying history, and selected files are embedded once per session (again only when they change); a crashed shim reports its stderr and returns the sidebar to idle.
...
- **Selected files are sent once per session.** A file in the Selected Files pane is embedded in the prompt the first time it is used and again only when its content changed; the agent already holds it in its context. Avante warns when a single turn embeds many files. The current buffer is only auto-added when it is a regular file, so a directory buffer (oil.nvim, netrw, `nvim .`) never pulls the whole project into the selection.
- **Compaction is the agent's.** Claude Code summarizes its own context when it fills up; the sidebar's `/compact` is forwarded to it. To change when that happens, set Claude Code's knobs in the provider `env`, e.g. `CLAUDE_CODE_AUTO_COMPACT_WINDOW = "500k"` (a token window, or `auto`), `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` (percentage of the window), or `DISABLE_AUTO_COMPACT = "1"`. Claude Code's own `/autocompact [auto|<tokens>]` command also works from the sidebar (e.g. `/autocompact 500000`); the agent stores the value in its settings, so it survives restarts.
